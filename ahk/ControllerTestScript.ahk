...
if ControllerNumber <= 0
{
    Loop 16  ; Query each controller number to find out which ones exist.
    {
        if GetKeyState(A_Index "JoyName")
        {
			...
			E.Value := "Controller Number is now " ControllerNumber
            break
        }
    }
    if ControllerNumber <= 0
	...
	Loop 16
	{
		...
		if buttons_down != ""
		{
			...
			if InStr(cont_info, "P")
				...
				E.Value .= "`n" cont_name " (#" ControllerNumber "):`n" axis_info "`nButtons Down: " buttons_down
		}
	}
    Sleep 100
}
